...
이 에이전트는 7-Phase 워크플로우의 **Phase 5 (TEST)**에서 자동 호출됩니다.

```
┌─────────────────────────────────────────────────────────┐
│ Phase 5: TEST                                           │
│                                                         │
│ 1. 개발 에이전트가 구현 완료 (Phase 4)                  │
│ 2. tester 에이전트가 테스트 실행                        │
│ 3. 실패 → 코드 수정 → 재테스트 (반복)                  │
│ 4. 통과 → Phase 6 (PR REVIEW)로 진행                   │
└─────────────────────────────────────────────────────────┘
```
...
```
1. tester가 실패 원인 분석 및 보고
2. 개발 에이전트 (ai-orchestrator-dev, core-api-dev, frontend-developer)에게 수정 요청
3. 개발 에이전트가 코드 수정
4. tester가 재테스트
5. 통과할 때까지 반복
```

### 통과 기준 (Phase 5 → Phase 6 전환 조건)
...
```
□ pytest -v (AI Orchestrator)
□ ./gradlew test (Core API)
□ npm test (UI - 해당 시)
□ 시나리오 테스트 (필요 시)
```
...
|------------|------|------------|
| 4. IMPLEMENT (완료) | 5. TEST | 6. PR REVIEW |
| 4. IMPLEMENT (완료) | 5. TEST (실패) | → 4. IMPLEMENT (수정) → 5. TEST |
