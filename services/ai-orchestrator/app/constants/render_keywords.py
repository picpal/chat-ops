...
# ============================================
# 암시적 차트 키워드 (명시적 키워드 없이도 차트 렌더링 트리거)
# ============================================

# 암시적 차트 키워드: 데이터 패턴과 함께 판단하여 차트 타입 결정
# 명시적 키워드("그래프", "차트")가 없어도 이 키워드가 있으면 차트로 렌더링 고려
# 우선순위: line > pie > bar (시계열 키워드가 가장 구체적이므로 먼저 체크)
IMPLICIT_CHART_KEYWORDS: Dict[str, List[str]] = {
    "line": ["추이", "추세", "변화", "트렌드", "일별", "월별", "시간대별", "주별", "연별"],
    "pie": ["분포", "비율", "점유율", "비중"],
    "bar": ["비교", "비교해", "현황"],
}

# 차트 렌더링이 부적합한 조건 (테이블로 강제 전환)
CHART_UNSUITABLE_CONDITIONS: Dict[str, int] = {
    "max_rows_for_chart": 10,       # 10행 초과 → 테이블
    "max_categories_for_pie": 7,    # pie 차트에서 7개 초과 → 테이블
}
